...
"""
    ...
    :param filename: the URL of pic
    :var filename: str
    :param d: the bytes of pic
    :var filename: byte
    :param lable_list: the list of lables
    :var lable_list: str
...
async def tag(d, lable_list):
    ...
    for _ in range(len(lable_list)):
        ...
        ymin = int(lable_list[_]["box"]["ymin"])
        ymax = int(lable_list[_]["box"]["ymax"])
        lable = lable_list[_]["label"]
        scores = lable_list[_]["score"]
        cv2.rectangle(
            img=img, pt1=(xmin, ymin), pt2=(xmax, ymax), color=(0, 255, 0), thickness=2
        )
        font = cv2.FONT_HERSHEY_SIMPLEX  # 定义字体
        imgzi = cv2.putText(
            img,
            "{} {:.3f}".format(lable, scores),
            (xmin, ymax - 20),
            font,
            1.2,
            (0, 255, 255),
