...
async def on_get_fursuit_rand(app: Ariadne, group: Group, message: MessageChain):
    if message.display == "来只毛":
        ...
        await app.send_message(
            group,
            MessageChain(["--- 每日吸毛 Bot ---\nID：{}\n毛毛名字：{}\n搜索方法：全局随机".format(jsonfy["data"]["id"],jsonfy["data"]["name"]),Image(data_bytes=await img_resp(jsonfy["data"]["url"]))]),
        )
@channel.use(
    ListenerSchema(
...
async def on_get_fursuit_by_name(app: Ariadne, group: Group, message: MessageChain, sparkle: Sparkle):
    ...
    if jsonfy['code'] == 200:
        await app.send_message(
            group,
            MessageChain(["--- 每日吸毛 Bot ---\nID：{}\n毛毛名字：{}\n搜索方法：全局模糊".format(jsonfy["data"]["id"],jsonfy["data"]["name"]),Image(data_bytes=await img_resp(jsonfy["data"]["url"]))]),
        )
    else:
        await app.send_message(
            group,
...
async def on_get_fursuit_by_id(app: Ariadne, group: Group, message: MessageChain, sparkle: Sparkle):
    lyrics1 = sparkle["furid"]
    jsonfy=Tailapi.getFursuitByID(str(lyrics1.result))
    if jsonfy['code']==200:
        await app.send_message(
            group,
            MessageChain(["--- 每日吸毛 Bot ---\nID：{}\n毛毛名字：{}\n搜索方法：全局精确".format(jsonfy["data"]["id"],jsonfy["data"]["name"]),Image(data_bytes=await img_resp(jsonfy["data"]["url"]))]),
        )
    else:
        await app.send_message(
            group,
