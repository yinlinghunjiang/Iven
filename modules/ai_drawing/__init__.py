...
async def ai_drawing(app: Ariadne, group: Group, message: MessageChain, member: Member):
    ...
    if member.id in queues:
        ...
        try:
            ret_msg = await inc.wait(TagWaiter(group, member), timeout=30)
            ...
            if res != "":
                await app.send_message(
                    group, MessageChain([At(member.id), " 绘制完成", Image(data_bytes=res)])
                )
                queue.pop(member.id)
            else:
                await app.send_message(
                    group, MessageChain([At(member.id), " 绘制失败，您可以在30s后重试"])
                )
                queue.pop(member.id)
